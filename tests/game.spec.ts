import {finishRide,waitForJob,waitForRideDone} from './ride-helper';
...
const legacyJudolSave = (state: State) => JSON.stringify({ ...state, story: { ...state.story, intro: 4, guide: 3, view: 'game' } });
async function start(page: Page, state: State = fresh()) {
  state.story.intro = 4; state.story.guide = 3; state.story.view = 'phone';
  await page.addInitScript(({ key, value }) => { if (!sessionStorage.getItem('seeded')) { localStorage.setItem(key, value); sessionStorage.setItem('seeded', 'yes'); } }, { key: SAVE_KEY, value: JSON.stringify(state) });
  await page.goto('/');
  await resume(page);
  await expect(page.locator('body')).toHaveAttribute('data-shell', 'playing');
}
async function openJudol(page: Page) {
  await page.locator('[data-story=judol]').click();
  await expect(page.locator('#loading')).toBeHidden();
  await expect(page.locator('.smartphone #spin')).toBeVisible();
}
async function tab(page: Page, name: 'workshop' | 'machine') {
  await page.locator('.smartphone footer [data-story=home]').click();
  await page.locator(name === 'workshop' ? '[data-story=shop]' : '[data-story=judol]').click();
}
...
test('renders assets, runs a spin, persists its outcome and has no viewport overflow', async ({ page }) => {
  ...
  await start(page); await openJudol(page); await expect(page.locator('canvas')).toHaveCount(1);
...
test('a pre-0.6 save that stored view game reopens Judol once', async ({ page }) => {
  const state = fresh();
  await page.addInitScript(({ key, value }) => { if (!sessionStorage.getItem('seeded')) { localStorage.setItem(key, value); sessionStorage.setItem('seeded', 'yes'); } }, { key: SAVE_KEY, value: legacyJudolSave(state) });
  await page.goto('/');
  await resume(page);
  await expect(page.locator('.smartphone.judol-phone')).toBeVisible();
  await expect(page.locator('.smartphone #spin')).toBeVisible();
  await page.reload();
  await resume(page);
  await expect(page.locator('.smartphone.judol-phone')).toHaveCount(0);
  await expect(page.locator('[data-story=judol]')).toBeVisible();
});
test('upgrade purchase unlocks exactly one held reel for one paid respin', async ({ page }) => {
  const initial=fresh();initial.upgrades.payout=1;await start(page,initial); await tab(page, 'workshop'); await page.locator('.smartphone [data-upgrade="hold"]').click();
...
test('auto-spin stops when the day runs out; sleep restores time without restarting auto',async({page})=>{
 ...
 await start(page,s);await openJudol(page);await page.locator('#auto').click();await expect(page.locator('#modal-body')).toContainText('Waktunya sudah habis');
...
test('Sultan unlock and mode switching update the real spin cost', async ({ page }) => {
  ...
  await start(page, s); await openJudol(page); await page.locator('#sultan-tab').click(); await page.locator('[data-action="unlock"]').click();
...
test('prestige requires a review, preserves insight, and resets the run', async ({ page }) => {
  ...
  await start(page, s); await page.locator('.smartphone footer [data-story=room]').click(); await page.locator('[data-story=restart]').click();
...
test('help and odds controls are functional and reduced motion still resolves a spin', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' }); await start(page); await openJudol(page);
  await page.locator('.smartphone #help').click(); await expect(page.locator('#modal-body')).toContainText('CARA BERMAIN');
  await page.locator('[data-action="close"]').click();
  await page.locator('.smartphone #odds').click(); await expect(page.locator('.odds-table tbody tr')).toHaveCount(6);
...
test('zero-cash rider pays once, reserves two hours, resumes its saved road',async({page})=>{
 await start(page,newRun());await page.locator('[data-story=work]').click();await page.locator('[data-action=start-job]').click();
...
test('rider keyboard and touch controls change lanes; completing work preserves upgrades',async({page})=>{
 ...
 await start(page,s);await page.locator('[data-story=work]').click();await page.locator('[data-action=start-job]').click();
...
test('loan offer shows interest before acceptance, then permits exact partial repayment', async ({ page }) => {
  await start(page); await page.locator('.smartphone footer [data-story=home]').click(); await page.locator('[data-story=finance]').click(); await page.locator('[data-action="offer"][data-amount="10000"]').click();
  ...
  await page.locator('[data-story=finance]').click(); await page.locator('[data-action="repay"][data-amount="5000"]').click();
...
test('due-day work is bounded by remaining time and a shortfall ends in repossession',async({page})=>{
 ...
 await start(page,s);await expect(page.locator('#spin')).toBeDisabled();await expect(page.locator('[data-action=work]')).toHaveCount(0);
...
test('cascade machine visibly resolves three multiplier stages with no hold', async ({ page }) => {
  ...
  await page.addInitScript(()=>{Math.random=()=>0;});await start(page,s); await openJudol(page);
...
test('day review pays a due loan automatically and a time upgrade extends the visible budget',async({page})=>{
 ...
 await start(page,s);await page.locator('#modal-body [data-action=day]').click();await page.locator('[data-action=end-day]').click();
 ...
 expect((await saved(page)).upgrades.stamina).toBe(1);await page.locator('.phone-close').click();await expect(page.locator('.day-bar')).toContainText('11j 0m');
});
...
test('rider pause freezes progress and a completed checkpoint survives refresh',async({page})=>{
 await start(page,newRun());await page.locator('[data-story=work]').click();await page.locator('[data-action=start-job]').click();
 await page.clock.install();await page.locator('#ride-go').click();
 await waitForJob(page,job=>job?.step===1);
...
test('guided ride teaches the first two sections then credits one shift', async ({ page }) => {
  ...
  await page.locator('[data-story=work]').click();
...
test('room, phone, and game cash match after one shift', async ({ page }) => {
  ...
  await page.locator('[data-story=judol]').click();
  await expect(page.locator('.smartphone #spin')).toBeVisible();
  await expect(page.locator('.smartphone .phone-cash')).toHaveText(phone);
  await expect(page.locator('.world-wallet b')).toHaveText(phone);
});
...
test('review screenshots for first-spin guide, paused ride, and mobile room', async ({ page }, info) => {
  ...
  await expect(page.locator('#loading')).toBeHidden();
  await expect(page.locator('#first-spin-tip')).toBeVisible();
  await expect(page.locator('.smartphone [data-story=system]')).toBeVisible();
  ...
  await page.locator('.smartphone footer [data-story=room]').click();
  await expect(page.locator('.room-stage')).toBeVisible();
